...
public class TransactionController {
	...
	public ResponseEntity<?> getLast100TransactionsByCustomerId(@RequestBody String request)
		...
		try {
			...
			Optional<List<Transaction>> listOptinal = transactionRepository.findTransactionsByCustomerId(customerId);
			if (!listOptinal.isPresent()){
				return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
			}
			return ResponseEntity.ok().body(gson.toJson(listOptinal.get()));
		} catch (Exception ex) {
	...
	public ResponseEntity<?> getMiniStatementByCustomerIdAndAccountNo(@RequestBody String request)
		...
		try {
			...
			return ResponseEntity.ok().body(gson.toJson(miniStatement.get()));
		} catch (Exception ex) {
			logger.info("Exception {}", AppUtilities.getExceptionStacktrace(ex));
